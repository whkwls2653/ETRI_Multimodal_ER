...
class COGMEN(nn.Module):
    def __init__(self, args):
        ...
        dataset_label_dict = {
            ...
            "KEMDy19": {  'neutral': 0,  'angry': 1,  'happy': 2,  'surprise': 3,  'sad': 4,  'fear': 5,  'disgust': 6},
        }
        ...
        self.rnn = SeqContext(u_dim, g_dim, args).to(self.device)
        self.gcn = GNN(g_dim, h1_dim, h2_dim, args).to(self.device)
        if args.concat_gin_gout:
            self.clf = Classifier(
                g_dim + h2_dim * args.gnn_nheads, hc_dim, tag_size, args
            )
        else:
            self.clf = (Classifier(h2_dim * args.gnn_nheads, hc_dim, tag_size, args)).to(self.device)
