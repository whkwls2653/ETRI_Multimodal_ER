...
def main(args):
    ...
    for path in annotations:
        gender=path.split('_')[-2]

        df = pd.read_csv(path)
        # print(df.loc[1:, "Wav":"Unnamed: 12"])
        ##use only speaker data
        df=df[df['Segment ID'].str.contains(gender,na=False)]
        # print('segment ID',df['Segment ID'])
    ...
    csv[csv['path_exist']].to_csv(os.path.join(args.save_path, 'annotation_speaker_only.csv'), index=False)
